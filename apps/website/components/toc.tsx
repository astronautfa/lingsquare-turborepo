          ...
          >
            <ul>
              {contents.map((e) => {
                if (e.depth > 2) return;
                return (
                  <li key={e.text} className="hover:text-sky-800">
...
const TocDesktop = ({ contents, className }: TocProps) => {
  ...
  return (
    <div className={className}>
      <nav className="pl-8">
        <p className="text-xl pb-4 font-medium">Table of Content</p>
            ...
            return (
              <li key={e.text}>
                <Link
                  href={`#${e.slug}`}
                  className={cn(
                    "block p-2 text-[--color-body] hover:text-sky-800 px-4 py-2",
