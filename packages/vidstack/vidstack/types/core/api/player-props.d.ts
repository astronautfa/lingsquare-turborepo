...
import type { ScreenOrientationLockType } from '../../foundation/orientation/types.js';
import type { GoogleCastOptions } from '../../providers/google-cast/types.js';
import type { MediaKeyShortcuts, MediaKeyTarget } from '../keyboard/types.js';
import type { MediaStorage } from '../state/media-storage.js';
import type { MediaState } from './player-state.js';
import type { MediaSrc } from './src-types.js';
import type { MediaLoadingStrategy, MediaPosterLoadingStrategy } from './types.js';
export declare const mediaPlayerProps: MediaPlayerProps;
export interface MediaStateAccessors extends Pick<MediaState, 'paused' | 'muted' | 'volume' | 'currentTime' | 'playbackRate'> {
}
export type PlayerSrc = MediaSrc | MediaSrc[];
export interface MediaPlayerProps extends Pick<Writable<MediaState>, 'artist' | 'artwork' | 'autoPlay' | 'clipStartTime' | 'clipEndTime' | 'controls' | 'currentTime' | 'loop' | 'muted' | 'paused' | 'playsInline' | 'poster' | 'preload' | 'playbackRate' | 'viewType' | 'volume' | 'title' | 'streamType' | 'liveEdgeTolerance' | 'minLiveDVRWindow'> {
