...
import { isString, isNumber, isFunction, isUndefined, waitTimeout, isArray, isBoolean, isNull, deferredPromise, listenEvent, scoped, getScope, EventsTarget, DOMEvent, State, fscreen, tick, createContext, useContext, Component, functionThrottle, isDOMNode, setAttribute, effect, isKeyboardClick, isTouchEvent, setStyle, onDispose, untrack, functionDebounce, ViewController, signal, peek, isKeyboardEvent, useDisposalBin, isNil, createScope, camelToKebabCase, waitIdlePeriod, provideContext, animationFrameThrottle, uppercaseFirstChar, computed, prop, method, noop, ariaBool as ariaBool$1, isWriteSignal, hasProvidedContext, isObject, useState, r, wasEnterKeyPressed, isPointerEvent, isMouseEvent, kebabToCamelCase, createDisposalBin, composeRefs, useStateContext, useSignal, useSignalRecord } from './vidstack-DcuYVyd0.js';
...
class TextTrack extends EventsTarget {
  ...
  addCue(cue, trigger) {
    ...
    for (i = 0; i < length; i++) if (cue.endTime <= this.C[i].startTime) break;
    if (i === length) this.C.push(cue);
    else this.C.splice(i, 0, cue);
  ...
  /** @internal */
  [(_c = TextTrackSymbol.na, _b = TextTrackSymbol.ib, _a$2 = TextTrackSymbol.$, TextTrackSymbol.Fb)](currentTime, trigger) {
    this.va = currentTime;
    if (this.mode === "disabled" || !this.C.length) return;
    const activeCues = [];
    ...
    this.Gb = activeCues;
    if (changed) this.Pf(trigger);
  }
  /** @internal */
  [TextTrackSymbol._]() {
    this._ = true;
    if (this.V !== "disabled") this.Qf();
  }
  $h(init) {
  ...
  async Qf() {
    if (!this._ || this[TextTrackSymbol.na] > 0) return;
  ...
  Ha() {
    ...
    if (!this.src || this.type !== "vtt") {
      ...
      if (native && !native.managed) {
        for (const cue of this.C) native.track.addCue(cue);
      }
    }
...
class MediaRemoteControl {
  ...
  changeDuration(duration, trigger) {
    this.t("media-duration-change-request", trigger, duration);
  }
  /**
  ...
  changeClipStart(startTime, trigger) {
    this.t("media-clip-start-change-request", trigger, startTime);
  }
  /**
  ...
  changeClipEnd(endTime, trigger) {
    this.t("media-clip-end-change-request", trigger, endTime);
  }
  /**
...
class NativeTextRenderer {
  ...
  attach(video) {
    this.m = video;
    if (video) video.textTracks.onchange = this.F.bind(this);
  }
  addTrack(track) {
    this.wa.add(track);
    this.di(track);
  ...
  detach() {
    ...
    for (const track of this.wa) this.removeTrack(track);
    this.wa.clear();
    this.m = null;
    this.K = null;
  }
  ...
  F(event) {
    for (const track of this.wa) {
      ...
      if (isShowing) this.Wf(track, native.track);
  ...
  Yf(video) {
    requestAnimationFrame(() => {
      ...
      if (video) {
        ...
        for (const track of this.Xd) this.Zf(track);
      }
      this.Ia();
    });
  }
...
class List extends EventsTarget {
  ...
  [(_a = ListSymbol.Zc, Symbol.iterator)]() {
  ...
  [ListSymbol.ea](item, trigger) {
    ...
    if (this.B.includes(item)) return;
    this.B.push(item);
  ...
  [ListSymbol.A](trigger) {
    for (const item of [...this.B]) this[ListSymbol.dc](item, trigger);
    this.B = [];
  ...
  [ListSymbol.Pd](readonly, trigger) {
    if (this[ListSymbol.Zc] === readonly) return;
...
class TextTrackList extends List {
    ...
    this.cg = functionDebounce(async () => {
      ...
      for (const kind of kinds) {
        ...
        if (track && (!isCaptionsKind || showCaptions !== false)) {
          track.mode = "showing";
          if (isCaptionsKind) this.dg(track);
        }
      }
    }, 300);
    this._d = null;
  ...
  add(init, trigger) {
    ...
    if (this.lb[kind] && init.default) delete init.default;
    ...
    if (this._) track[TextTrackSymbol._]();
    if (init.default) this.lb[kind] = track;
    this.cg();
    return this;
  }
  remove(track, trigger) {
    this._d = track;
    if (!this.B.includes(track)) return;
    if (track === this.lb[track.kind]) delete this.lb[track.kind];
    track.mode = "disabled";
  ...
  [(TextTrackSymbol._)]() {
    if (this._) return;
    for (const track of this.B) track[TextTrackSymbol._]();
    this._ = true;
    this.cg();
  }
  ii(event) {
...
class VideoQualityList extends SelectList {
   ...
   */
  autoSelect(trigger) {
    if (this.readonly || this.dd || !this[QualitySymbol.Ja]) return;
...
class MediaControls extends MediaPlayerController {
  ...
  ai() {
    ...
    if (!shouldHideOnMouseLeave || !this.Ud()) {
      effect(() => {
        if (!paused()) this.listen("pointermove", this.Uf.bind(this));
      });
    }
...
class FullscreenController extends ViewController {
  ...
  onConnect() {
    listenEvent(fscreen, "fullscreenchange", this.F.bind(this));
    listenEvent(fscreen, "fullscreenerror", this.R.bind(this));
  ...
  F(event) {
    ...
    if (active === this.Qd) return;
    if (!active) this.ec = false;
    this.Qd = active;
...
const _ScreenOrientationController = class _ScreenOrientationController extends ViewController {
  ...
  async Ga() {
    if (this.supported && this.Db()) await this.unlock();
  }
  Lf(event) {
    this.ma.set(this.Kf());
   ...
   */
  async lock(lockType) {
    if (peek(this.Db) || this.$c === lockType) return;
    this.Mf();
...
class AudioProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-D_S87JW-.js')).AudioProvider(this.target, ctx);
  }
}
...
const _HLSProviderLoader = class _HLSProviderLoader extends VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import('./vidstack-BQz17T13.js')).HLSProvider(this.target, context);
  }
};
...
class PageVisibility {
  constructor() {
    this.Aa = signal(determinePageState());
    this.Xn = signal(
      IS_SERVER ? "visible" : document.visibilityState
    );
  }
  connect() {
    for (const eventType of PAGE_EVENTS) {
      listenEvent(window, eventType, this._n.bind(this));
    }
    if (IS_SAFARI) {
  ...
  _n(event) {
    if (IS_SAFARI) window.clearTimeout(this.Yn);
    if (event.type !== "blur" || this.Aa() === "active") {
      this.Aa.set(determinePageState(event));
      this.Xn.set(document.visibilityState == "hidden" ? "hidden" : "visible");
    }
  }
}
...
class RAFLoop {
  ...
  Ya() {
    if (!isUndefined(this.za)) return;
    this.gg();
  }
  aa() {
    if (isNumber(this.za)) window.cancelAnimationFrame(this.za);
    this.za = void 0;
  }
  gg() {
...
class HTMLMediaEvents {
  ...
  lc() {
    ...
    const didStutter = IS_SAFARI && newTime - this.Un < 0.35;
    if (!didStutter && this.Tn !== newTime) {
      this.Kb(newTime);
      this.Tn = newTime;
    }
  }
  ...
  jb(event2) {
    if (this.a.readyState === 1 && !this.kc) return;
    this.kc = false;
    this.ga.aa();
  ...
  oi() {
    const isPaused = this.b.$state.paused(), isPageHidden = this.Zn.visibility === "hidden", shouldListenToTimeUpdates = isPaused || isPageHidden;
...
class NativeAudioTracks {
  ...
  Di(event) {
    ...
    if (track) this.b.audioTracks[ListSymbol.dc](track, event);
  }
  Ei(event) {
    let enabledTrack = this.jg();
    if (!enabledTrack) return;
    const track = this.b.audioTracks.getById(enabledTrack.id);
    if (track) this.b.audioTracks[ListSymbol.fa](track, true, event);
  }
  jg() {
...
class HTMLRemotePlaybackAdapter {
  ...
  ie() {
    if (IS_SERVER || !this.a?.remote || !this.lg) return;
...
class NativeHLSTextTracks {
  ...
  Zd(event) {
    ...
    if (!nativeTrack || findTextTrackElement(this.m, nativeTrack)) return;
...
class VideoProvider extends HTMLMediaProvider {
  ...
  setup() {
    ...
    if (this.type === "video") this.b.delegate.c("provider-setup", this);
  }
  /**
...
class DASHController {
  ...
  Mi(event) {
    ...
    if (isNumber(videoQuality.index)) {
      ...
      if (quality) this.b.qualities[ListSymbol.fa](quality, true, trigger);
    }
...
class DASHLibLoader {
  ...
  async re() {
    ...
    if (isUndefined(ctor) && !isString(this.M)) ctor = await importDASH(this.M, callbacks);
    if (!ctor) return null;
...
class VimeoProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-p3Pf89tc.js')).VimeoProvider(this.target, ctx);
  }
...
class YouTubeProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-CwOSgZFo.js')).YouTubeProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
...
class RequestQueue {
  ...
  tg() {
    for (const key of this.i.keys()) this.ye(key);
    this.ug();
  }
  ug() {
    this.ze.resolve();
...
class MediaRequestManager extends MediaPlayerController {
  ...
  onDestroy() {
    try {
      ...
      if (fullscreen()) this.Mg("prefer-media", destroyEvent);
      if (pictureInPicture()) this.He(destroyEvent);
    } catch (e) {
    }
    this.Ac.A();
  }
  ...
  Ij(event) {
    ...
    if (peek(this.D)) {
      ...
      this.Ac.k(event.type, () => {
        if (peek(this.D)) this[event.type](event);
      });
    }
  }
  async Bc(trigger) {
  ...
  async Pg(trigger) {
    try {
      ...
      if (!this.rd) {
        const $module = await import('./vidstack-BO-L-6zu.js').then(function (n) { return n.loader; });
  ...
  async Rd(event) {
    ...
    if (isUndefined(lockType) || lockType === "none" || !this.cb.supported) return;
    if (isFullscreen) {
...
class MediaStateManager extends MediaPlayerController {
  ...
  W(event) {
    ...
    if (!IS_SERVER) {
      if (TRACKED_EVENT.has(type)) this.v.set(type, event);
      this.dispatch(event);
    }
  }
  Vj() {
  ...
  Sg(event) {
    if (event) this.E("media-text-track-change-request", event);
  ...
  Zj(event) {
    ...
    if (event) this.E("media-audio-track-change-request", event);
  ...
  _a(event) {
    ...
    if (event) this.E("media-quality-change-request", event);
  ...
  ["time-change"](event) {
    ...
    for (const track of this.a.textTracks) {
      track[TextTrackSymbol.Fb](newTime, event);
    }
  ...
  Wn() {
    ...
    if (paused()) return;
    this.Sn = updateTimeIntervals(
      this.Vn,
      this.Sn,
      currentTime()
    );
    played.set(new TimeRange(this.Vn));
  }
...
const _MediaPlayer = class _MediaPlayer extends Component {
  ...
  onAttach(el) {
    ...
    if (IS_SERVER) this.ud();
    else effect(this.ud.bind(this));
    if (IS_SERVER) this.Zg();
    else effect(this.Zg.bind(this));
  ...
  tk() {
    if (this.$state.canPlay() && this.p) this.canPlayQueue.Ya();
    else this.canPlayQueue.aa();
  }
  sk() {
  ...
  $g(paused) {
    ...
    } else this.canPlayQueue.k("paused", () => this.X.Bc());
  }
  get muted() {
  ...
  Ck() {
    ...
    if (noMatch) {
      ...
      Promise.all(
        ...
        const newSource2 = this.eh(peek($state.source), sources2);
        tick();
        if (!newSource2.src) {
          this.c("error", {
...
class MediaProvider extends Component {
  ...
  Hk(target) {
    ...
    if (this.Y === loader && loader?.target === target && peek($provider)) return;
    this.hh();
    this.Y = loader;
...
class SliderEventsController extends ViewController {
  ...
  onConnect() {
    ...
    if (this.j.lh) effect(this.Zk.bind(this));
  }
  Zk() {
  ...
  $k(event) {
    if (isNull(this.db) || isTouchPinchEvent(event)) return;
  ...
  Yk() {
    if (this.j.z() || !this.$state.dragging()) return;
  ...
  ic(event) {
    const isValidKey = Object.keys(SliderKeyDirection).includes(event.key);
    if (!isValidKey || !isNull(this.Ln(event))) return;
...
class SliderController extends ViewController {
  ...
  onAttach(el) {
    ...
    if (IS_SERVER) this.ph();
    else effect(this.ph.bind(this));
  }
  onConnect(el) {
...
class Thumbnail extends Component {
  ...
  pl() {
    for (const reset of this.kf) reset();
    this.kf = [];
  }
}
Thumbnail.props = {
...
class TimeSlider extends Component {
  ...
  Ed(event) {
    ...
    if (!peek(seeking)) this.Ka(this.Xb(event.detail), event);
...
class SliderChapters extends Component {
  ...
  pf(start, end, percent) {
    for (let i = start; i < end; i++) this.Eh(this.ka[i], percent);
  }
  Eh(ref, percent) {
    if (!ref) return;
  ...
  qc() {
    ...
    this.zb = this.Sl();
    if (this.zb) effect(this.Tl.bind(this));
    return () => {
      if (this.zb) {
        this.zb.textContent = "";
        this.zb = null;
  ...
  Tl() {
    ...
    if (this.zb) this.zb.textContent = cue?.text || "";
  }
  Ul() {
    let node = this.el;
...
class Menu extends Component {
  ...
  Zl(isExpanded, event) {
    ...
    event?.stopPropagation();
    if (this.U() === isExpanded) return;
    if (this.z()) {
      if (isExpanded) this.Ld.hide(event);
      return;
    }
    ...
    if (content) setAttribute(content, "aria-labelledby", isExpanded && this.uf);
    ...
    if (isExpanded) {
      ...
      if (this.isSubmenu) {
        for (const el of this.Wc) el.close(event);
      } else {
  ...
  gm(trigger) {
    ...
    if (this.U()) this.a.remote.pauseControls(trigger);
  ...
  fm(event) {
    ...
    for (const callback of this.Nd) callback(event);
  }
  open(trigger) {
    if (peek(this.U)) return;
...
class RadioGroupController extends ViewController {
  ...
  Oh(newValue) {
    for (const radio of this.ac) {
      if (newValue === peek(radio.Ua)) return radio;
    }
    return null;
  }
}
...
class Radio extends Component {
  ...
  s(event) {
    if (peek(this.Ab)) return;
    this.F(true, event);
    this.vm(event);
    this.e.Od?.(peek(this.$props.value), event);
  }
  Xc(value, trigger) {
    if (peek(this.Ab) === value) return;
...
class Time extends Component {
  ...
  onAttach(el) {
    if (!el.hasAttribute("role")) effect(this.Km.bind(this));
