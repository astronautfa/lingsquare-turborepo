...
class TextTrack extends EventsTarget {
  ...
  /** @internal */
  [(_c = TextTrackSymbol._readyState, _b = TextTrackSymbol._onModeChange, _a$2 = TextTrackSymbol._native, TextTrackSymbol._updateActiveCues)](currentTime, trigger) {
...
class AudioProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-BMGEnwTE.js')).AudioProvider(this.target, ctx);
  }
}
...
const _HLSProviderLoader = class _HLSProviderLoader extends VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import('./vidstack-DRzO1Gfq.js')).HLSProvider(this.target, context);
  }
};
...
class PageVisibility {
  constructor() {
    this._state = signal(determinePageState());
    this._visibility = signal(
...
class VimeoProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-nCwDmB8r.js')).VimeoProvider(this.target, ctx);
  }
...
class YouTubeProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await import('./vidstack-B2_DFO9k.js')).YouTubeProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
  ...
  async _requestGoogleCast(trigger) {
    try {
      ...
      if (!this._googleCastLoader) {
        const $module = await import('./vidstack-B6Hn_Kfu.js').then(function (n) { return n.loader; });
