...
import {
  ...
  setARIALabel,
  setAttributeIfEmpty
} from "./vidstack-LJXEKMTP.js";
import {
  TimeRange,
  getTimeRangesEnd,
  getTimeRangesStart,
  updateTimeIntervals
} from "./vidstack-VZNFT54D.js";
import {
  IS_CHROME,
  IS_IOS,
  IS_IPHONE,
...
function softResetMediaState($media, isSourceQualityChange = false) {
  ...
  mediaState.reset($media, (prop11) => filter.has(prop11));
  tick();
}
...
var MediaRemoteControl = class {
  ...
  #noPlayerWarning(method8) {
    if (true) {
      console.warn(
        `[vidstack] attempted to call \`MediaRemoteControl.${method8}\`() that requires player but failed because remote could not find a parent player element from target`
      );
    }
  }
};
...
var MediaKeyboardController = class extends MediaPlayerController {
  ...
  #onKeyUp(event) {
    ...
    let { method: method8, value } = this.#getMatchingMethod(event);
    ...
    if (method8?.startsWith("seek")) {
      event.preventDefault();
      event.stopPropagation();
      if (this.#timeSlider) {
        this.#forwardTimeKeyboardEvent(event, method8 === "seekForward");
    ...
    if (method8?.startsWith("volume")) {
      const volumeSlider = this.el.querySelector("[data-media-volume-slider]");
      volumeSlider?.dispatchEvent(
        new KeyboardEvent("keyup", {
          key: method8 === "volumeUp" ? "Up" : "Down",
  ...
  #onKeyDown(event) {
    ...
    let { method: method8, value } = this.#getMatchingMethod(event), isNumberPress = !event.metaKey && /^[0-9]$/.test(event.key);
    ...
    switch (method8) {
      case "seekForward":
      case "seekBackward":
        this.#seeking(event, method8, method8 === "seekForward");
        break;
      case "volumeUp":
      case "volumeDown":
        const volumeSlider = this.el.querySelector("[data-media-volume-slider]");
        if (volumeSlider) {
          ...
          this.#media.remote.changeVolume(
            this.$state.volume() + (method8 === "volumeUp" ? +value2 : -value2),
            event
          );
        }
        break;
      ...
      case "slowDown":
        ...
        this.#media.remote.changePlaybackRate(
          Math.max(0.25, Math.min(2, playbackRate + (method8 === "speedUp" ? 0.25 : -0.25))),
          event
        );
        break;
      default:
        this.#media.remote[method8]?.(event);
    }
  ...
  #getMatchingMethod(event) {
    ...
    const method8 = Object.keys(keyShortcuts).find((method9) => {
      const value = keyShortcuts[method9], keys = isArray(value) ? value.join(" ") : isString(value) ? value : value?.keys;
    ...
    return {
      method: method8,
      value: method8 ? keyShortcuts[method8] : null
    };
  }
  #seekTotal;
...
var AudioProviderLoader = class {
  ...
  async load(ctx) {
    ...
    return new (await import("./vidstack-HERZEB4T.js")).AudioProvider(this.target, ctx);
  }
};
...
var VideoProviderLoader = class {
  ...
  async load(ctx) {
    ...
    return new (await import("./vidstack-Q352LV33.js")).VideoProvider(this.target, ctx);
  }
};
...
var HLSProviderLoader = class _HLSProviderLoader extends VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import("./vidstack-GWG6RLGQ.js")).HLSProvider(this.target, context);
  }
};
...
var DASHProviderLoader = class _DASHProviderLoader extends VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import("./vidstack-6BV5ERT4.js")).DASHProvider(this.target, context);
  }
};
...
var VimeoProviderLoader = class {
  ...
  async load(ctx) {
    ...
    return new (await import("./vidstack-SC7WDS4J.js")).VimeoProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
    const { resolveVimeoVideoId, getVimeoVideoInfo } = await import("./vidstack-6BO7KF4K.js");
...
var YouTubeProviderLoader = class {
  ...
  async load(ctx) {
    ...
    return new (await import("./vidstack-QM6G2ZKA.js")).YouTubeProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
    const { findYouTubePoster, resolveYouTubeVideoId } = await import("./vidstack-IC5UCU6E.js");
  ...
  async requestGoogleCast(trigger) {
    try {
      ...
      if (!this.#googleCastLoader) {
        const $module = await import("./vidstack-QIFF363N.js");
...
var MediaStateSync = class extends MediaPlayerController {
  ...
  #init() {
    ...
    for (const prop11 of Object.keys(this.$props)) {
      if (skip.has(prop11)) continue;
      this.$state[providedProps[prop11] ?? prop11]?.set(this.$props[prop11]());
    }
...
var MediaPlayer = class _MediaPlayer extends Component {
  ...
  #setupMediaAttributes() {
    ...
    for (const prop11 of mediaAttributes) {
      const attrName = "data-" + (alias[prop11] ?? camelToKebabCase(prop11));
      $attrs[attrName] = function() {
        return this.$state[prop11]();
      };
    }
    delete $attrs.title;
  ...
  #onMutation() {
    ...
    for (const el of children) {
      ...
      if (el instanceof HTMLSourceElement) {
        ...
        for (const prop11 of ["id", "src", "width", "height", "bitrate", "codec"]) {
          const value = el.getAttribute(`data-${prop11}`);
          if (isString(value)) src[prop11] = /id|src|codec/.test(prop11) ? value : Number(value);
        }
        sources.push(src);
...
var Menu = class extends Component {
  ...
  #onResize = animationFrameThrottle(() => {
    ...
    for (const prop11 of ["paddingTop", "paddingBottom", "borderTopWidth", "borderBottomWidth"]) {
      height += parseFloat(styles[prop11]) || 0;
    }
...
var Gesture = class extends Component {
  ...
  #performAction(action, trigger) {
    ...
    const [method8, value] = action.replace(/:([a-z])/, "-$1").split(":");
    if (action.includes(":fullscreen")) {
      ...
      this.#media.remote[kebabToCamelCase(method8)](trigger);
    }
